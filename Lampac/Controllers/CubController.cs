        ...
        async public Task Index()
        {
            ...
            if (HttpMethods.IsGet(HttpContext.Request.Method))
            {
                ...
                if (!hybridCache.TryGetValue(memkey, out (byte[] array, int statusCode, string contentType) cache))
                {
                    ...
                    var result = await Engine.CORE.HttpClient.BaseDownload($"{init.scheme}://{domain}/{uri}", timeoutSeconds: 10, proxy: proxy, headers: headers, statusCodeOK: false, useDefaultHeaders: false);
                ...
                if ((domain.StartsWith("tmdb") || domain.StartsWith("tmapi") || domain.StartsWith("apitmdb")) && cache.array.Length == 16)
                {
                    if (Encoding.UTF8.GetString(cache.array) == "{\"blocked\":true}")
                    {
                        ...
                        string json = await Engine.CORE.HttpClient.Get($"http://{AppInit.conf.localhost}:{AppInit.conf.listenport}/tmdb/api/{uri}", timeoutSeconds: 5, headers: header);
