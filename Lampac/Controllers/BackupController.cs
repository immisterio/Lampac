...
namespace Lampac.Controllers
{
    public class BackupController : BaseController
    {
        ...
        {
            string path = getFilePath(account_email, token, uid, false);
        ...
        [HttpPost]
        [Route("/backup/export")]
        async public Task<ActionResult> Export([FromQuery]string account_email, [FromQuery]string token, [FromQuery]string uid, IFormFile file)
        {
            if (file == null || file.Length == 0)
                Content("{\"secuses\": false, \"msg\": \"file\"}", "application/json; charset=utf-8");

            string path = getFilePath(account_email, token, uid, true);
            if (path == null)
                Content("{\"secuses\": false, \"msg\": \"path\"}", "application/json; charset=utf-8");
        ...
        #region getFilePath
        string getFilePath(string account_email, string token, string uid, bool createDirectory)
        {
            string id = token;
