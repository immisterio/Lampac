    ...
    public class Accsdb
    {
        ...
        public Task Invoke(HttpContext httpContext)
        {
            ...
            if (AppInit.conf.accsdb.enable)
            {
                ...
                if (httpContext.Request.Path.Value != "/" && !Regex.IsMatch(httpContext.Request.Path.Value, "^/((api/chromium|proxy-dash|ts|ws|headers|myip|geo|version|weblog|stats|admin|rch|merchant/payconfirm|bind|cub)(/|$)|(ping|extensions|kit)$|on/|(lite|online|sisi|timecode|bookmark|sync|tmdbproxy|dlna|ts|tracks|transcoding|backup|invc-ws)/js/|(streampay|b2pay|cryptocloud|freekassa|litecoin)/|lite/(withsearch|filmixpro|fxapi/lowlevel/|kinopubpro|vokinotk|rhs/bind|iptvonline/bind|getstv/bind)|([^/]+/)?app\\.min\\.js|([^/]+/)?css/app\\.css|[a-zA-Z\\-]+\\.js|msx/start\\.json|samsung\\.wgt)", RegexOptions.IgnoreCase))
                {
                    bool limitip = false;
