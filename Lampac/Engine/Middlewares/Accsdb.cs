        ...
        public Task Invoke(HttpContext httpContext)
        {
            ...
            if (AppInit.conf.accsdb.enable)
            {
                ...
                if (httpContext.Request.Path.Value != "/" && !Regex.IsMatch(httpContext.Request.Path.Value, jacpattern) && 
                    !Regex.IsMatch(httpContext.Request.Path.Value, "^/((ts|ws)(/|$)|extensions|(b2pay|cryptocloud|freekassa|litecoin)/|lite/(filmixpro|kinopubpro|vokinotk)|lampa-(main|lite)/app\\.min\\.js|[a-zA-Z]+\\.js|msx/start\\.json|samsung\\.wgt)"))
                {
                    bool limitip = false;
