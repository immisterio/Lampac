        ...
        async public Task<ActionResult> Index(string account_email, string imdb_id, string title, string original_title, string uri, long animeid, string t, bool rjson = false)
        {
            ...
            else 
            {
                ...
                if (!memoryCache.TryGetValue(memKey, out JArray voices))
                {
                    ...
                    voices = await HttpClient.Get<JArray>($"https://moonanime.art/content/anime/src/player/play/{animeid}/e/1/get_file:anime.playlist.txt?player=partner", timeoutSeconds: 10, httpversion: 2, proxy: proxyManager.Get(), headers: httpHeaders(init, HeadersModel.Init(
                        ("cache-control", "no-cache"),
                        ("dnt", "1"),
                        ("accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7"),
                        ("pragma", "no-cache"),
