...
{
    public class VidLink : BaseOnlineController
    {
        [HttpGet]
        [Route("lite/vidlink")]
        async public Task<ActionResult> Index(bool checksearch, long id, string imdb_id, string title, string original_title, int serial, int s = -1, bool rjson = false)
        {
            ...
            if (serial == 1)
            {
                ...
                if (s == -1)
                {
                    ...
                    foreach (var season in tmdb.Value)
                    {
                        ...
                        string link = $"{host}/lite/vidlink?id={id}&imdb_id={imdb_id}&serial=1&rjson={rjson}&title={HttpUtility.UrlEncode(title)}&original_title={HttpUtility.UrlEncode(original_title)}&s={number}";
                ...
                else
                {
                    ...
                    foreach (var season in tmdb.Value)
                    {
                        ...
                        for (int i = 1; i <= season.Value<int>("episode_count"); i++)
                            etpl.Append($"{i} серия", title ?? original_title, s.ToString(), i.ToString(), accsArgs($"{host}/lite/vidlink/video?id={id}&s={s}&e={i}"), vast: init.vast);
                    }
            ...
            else
            {
                ...
                mtpl.Append("1080p", accsArgs($"{host}/lite/vidlink/video?id={id}"), vast: init.vast);
