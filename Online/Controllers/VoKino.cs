...
namespace Lampac.Controllers.LITE
{
    public class VoKino : BaseOnlineController
    {
        ...
        async public Task<ActionResult> Index(long kinopoisk_id, string title, string original_title, string balancer, string t, int s = -1, bool rjson = false)
        {
            ...
            var cache = await InvokeCache<EmbedModel>(rch.ipkey($"vokino:{kinopoisk_id}:{balancer}:{t}", proxyManager), cacheTime(20, init: init), init.rhub ? null : proxyManager, async res =>
            {
            ...
            return OnResult(cache, () => oninvk.Html(cache.Value, kinopoisk_id, title, original_title, balancer, t, s, rjson));
        }
    }
}
