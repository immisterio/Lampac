...
namespace Lampac.Controllers.LITE
{
    ...
    {
        [Route("lite/jac")]
        async public Task<ActionResult> Index(string title, string original_title, int year, int quality = -1)
        {
            string memkey = $"lite/jac:{title}:{original_title}:{year}";
            if (!memoryCache.TryGetValue(memkey, out JArray results) || quality == -1)
            {
                var root = await HttpClient.Get<JObject>($"{AppInit.Host(HttpContext)}/api/v2.0/indexers/all/results?title={HttpUtility.UrlEncode(title)}&original_title={HttpUtility.UrlEncode(original_title)}&year={year}", timeoutSeconds: 8);
