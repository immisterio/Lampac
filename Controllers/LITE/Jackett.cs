...
namespace Lampac.Controllers.LITE
{
    public class Jackett : BaseController
    {
        [HttpGet]
        [Route("lite/jac")]
        async public Task<ActionResult> Index(string apikey, string title, string original_title, string original_language, int year, int serial, int quality = -1)
        {
            string memkey = $"lite/jac:{title}:{original_title}:{year}";
            if (!memoryCache.TryGetValue(memkey, out JArray results) || quality == -1)
            {
                var root = await HttpClient.Get<JObject>($"{AppInit.Host(HttpContext)}/api/v2.0/indexers/all/results?apikey={apikey}&title={HttpUtility.UrlEncode(title)}&title_original={HttpUtility.UrlEncode(original_title)}&year={year}&is_serial={(original_language == "ja" ? 5 : (serial + 1))}", timeoutSeconds: 11);
                if (root == null)
