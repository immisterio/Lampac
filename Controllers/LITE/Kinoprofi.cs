...
namespace Lampac.Controllers.LITE
{
    public class Kinoprofi : BaseController
    {
        ...
        async public Task<ActionResult> Index(string title, int year, int is_serial, int serial, string newsuri, string session, int s = -1)
        {
            ...
            if (is_serial == 2 || serial == 1)
            {
                    ...
                    if (!memoryCache.TryGetValue(memKey, out List<(string name, string uri)> links))
                    {
                        ...
                        foreach (string row in Regex.Replace(search, "[\n\r\t]+", "").Split("sh-block ns").Skip(1).Reverse())
                        {
                            ...
                            if (g[2].Value.Trim().ToLower() == title.ToLower())
                            {
                                ...
                                links.Add((g[3].Value.ToLower(), $"{AppInit.Host(HttpContext)}/lite/kinoprofi?title={HttpUtility.UrlEncode(title)}&is_serial={is_serial}&serial={serial}&s={g[4].Value}&newsuri={HttpUtility.UrlEncode(g[1].Value)}&session={session_id}"));
                            }
                        }
                ...
                else
                {
                    ...
                    foreach (var l in links)
                    {
                        ...
                        html += "<div class=\"videos__item videos__movie selector " + (firstjson ? "focused" : "") + "\" media=\"\" s=\"" + s + "\" e=\"" + Regex.Match(l.name, "^([0-9]+)").Groups[1].Value + "\" data-json='{\"method\":\"play\",\"url\":\"" + link + "\",\"title\":\"" + title + "\"}'><div class=\"videos__item-imgbox videos__movie-imgbox\"></div><div class=\"videos__item-title\">" + l.name + "</div></div>";
                        firstjson = true;
                    }
                    #endregion
                }
