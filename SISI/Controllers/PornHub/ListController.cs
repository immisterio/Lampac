...
namespace Lampac.Controllers.PornHub
{
    ...
    {
        public static List<(string name, string val)> httpheaders = new List<(string name, string val)>()
        {
        ...
        async public Task<JsonResult> Index(string search, string sort, int pg = 1)
        {
            ...
            if (!memoryCache.TryGetValue(memKey, out List<PlaylistItem> playlists))
            {
                ...
                string html = await PornHubTo.InvokeHtml(AppInit.conf.PornHub.host, search, sort, pg, url => HttpClient.Get(url, timeoutSeconds: 10, proxy: proxy, httpversion: 2, addHeaders: httpheaders));
                if (html == null)
