...
namespace Lampac.Controllers.XvideosRED
{
    public class ViewController : BaseSisiController
    {
        ...
        async public Task<JsonResult> Index(string uri, bool related)
        {
            ...
            {
                stream_links = await XvideosTo.StreamLinks($"{host}/xdsred/vidosik", init.corsHost(), uri, url => HttpClient.Get(url, cookie: init.cookie, timeoutSeconds: 10, proxy: proxy, headers: httpHeaders(init)));
